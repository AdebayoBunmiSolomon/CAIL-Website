...
export const EngineeringClaimRequiredDocumentsDetails: React.FC<
  useFormProps
> = ({ useFormProps }) => {
  const props = useFormProps;
  const { globalFileData, setGlobalFileData } = useGlobalFileStore();
  const { setEngineeringClaimFormData, engineeringClaimFormData } =
    useEngineeringClaimForm();

  useEffect(() => {
    if (engineeringClaimFormData.doYouHaveAWitness === "Yes") {
      props?.setValues("eyeWitnessReport", null);
    } else {
      props?.setValues("eyeWitnessReport", "NULL");
    }
  }, [engineeringClaimFormData.doYouHaveAWitness]);

  useEffect(() => {
    if (engineeringClaimFormData.hasThePoliceBeenInformed === "Yes") {
    ...
    <>
      <div className='flex justify-center items-center'>
        <div className='w-[95%] bg-white rounded-md self-center p-6'>
          <FormTitle title='Engineering Claim Required Documents' />
          ...
          <div className='flex flex-col md:flex-col lg:flex-row items-center gap-4 mb-3'>
            {engineeringClaimFormData.doYouHaveAWitness === "Yes" && (
              <Controller
                control={props?.control}
                render={({ field }) => (
                  <FileInput
            ...
            {engineeringClaimFormData.hasThePoliceBeenInformed === "Yes" && (
              <Controller
                control={props?.control}
                render={({ field }) => (
                  <FileInput
